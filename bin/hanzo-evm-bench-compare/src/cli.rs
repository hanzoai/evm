...
use hanzo_evm_node_core::args::{DatadirArgs, LogArgs, TraceArgs};
use hanzo_evm_tracing::FileWorkerGuard;
...
/// Automated evm benchmark comparison between git references
#[derive(Debug, Parser)]
#[command(
    name = "evm-bench-compare",
    about = "Compare evm performance between two git references (branches or tags)",
    version
)]
pub(crate) struct Args {
    ...
    #[arg(long, value_name = "PATH", default_value = "./evm-bench-compare")]
    ...
    /// Run evm binary with sudo (for elevated privileges)
    #[arg(long)]
    pub sudo: bool,
    ...
    /// Optional fixed delay between engine API calls (passed to evm-bench).
    ///
    ...
    /// Wait for blocks to be persisted before sending the next batch (passed to evm-bench).
    ///
    /// When enabled, waits for every Nth block to be persisted using the
    /// `evm_subscribePersistedBlock` subscription. This ensures the benchmark
    ...
    /// Engine persistence threshold (passed to evm-bench).
    ///
    ...
    /// Additional arguments to pass to baseline evm node command
    ///
    /// Example: `--baseline-args "--debug.tip 0xabc..."`
    #[arg(long, value_name = "ARGS")]
    pub baseline_args: Option<String>,

    /// Additional arguments to pass to feature evm node command
    ///
    /// Example: `--feature-args "--debug.tip 0xdef..."`
    #[arg(long, value_name = "ARGS")]
    pub feature_args: Option<String>,

    /// Additional arguments to pass to evm node command (applied to both baseline and feature)
    ///
    /// All arguments after `--` will be passed directly to the evm node command.
    /// Example: `evm-bench-compare --baseline-ref main --feature-ref pr/123 -- --debug.tip
    /// 0xabc...`
    #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
    pub evm_args: Vec<String>,

    /// Comma-separated list of features to enable during evm compilation (applied to both builds)
    ///
...
pub(crate) async fn run_comparison(args: Args, _ctx: CliContext) -> Result<()> {
    ...
    if args.sudo {
        info!("Running in sudo mode - evm commands will use elevated privileges");
    }
...
async fn run_compilation_phase(
    ...
    compilation_manager.ensure_hanzo_evm_bench_available()?;
    if args.profile {
...
async fn run_warmup_phase(
    ...
    // Start evm node for warmup (command is not stored for warmup phase)
...
async fn run_benchmark_workflow(
    ...
    for (i, &git_ref) in refs.iter().enumerate() {
        ...
        // Start evm node and capture the command for reporting
        let (mut node_process, evm_command) =
        ...
        // Note: evm-bench has an off-by-one error where it consumes the first block
        ...
        // Set the benchmark run timestamps and evm command
        comparison_generator.set_ref_timestamps(ref_type, benchmark_start, benchmark_end)?;
        comparison_generator.set_ref_command(ref_type, evm_command)?;
...
async fn generate_comparison_charts(comparison_generator: &ComparisonGenerator) -> Result<()> {
    ...
    let script_path = "bin/evm-bench/scripts/compare_newpayload_latency.py";
