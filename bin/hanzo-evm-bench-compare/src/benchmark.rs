//! Benchmark execution using evm-bench.
...
/// Manages benchmark execution using evm-bench
...
impl BenchmarkRunner {
    ...
    pub(crate) async fn run_warmup(&self, from_block: u64) -> Result<()> {
        ...
        // Build the evm-bench command for warmup (no output flag)
        let mut cmd = Command::new("evm-bench");
        cmd.args([
            "new-payload-fcu",
            "--rpc-url",
            &self.rpc_url,
        ...
        debug!("Executing warmup evm-bench command: {:?}", cmd);

        // Execute the warmup benchmark
        let mut child = cmd.spawn().wrap_err("Failed to start warmup evm-bench process")?;
        ...
        let status = child.wait().await.wrap_err("Failed to wait for warmup evm-bench")?;

        if !status.success() {
            return Err(eyre!("Warmup evm-bench failed with exit code: {:?}", status.code()));
        }
    ...
    pub(crate) async fn run_benchmark(
        ...
        // Create log file path for evm-bench output
        let log_file_path = output_dir.join("hanzo_evm_bench.log");
        info!("evm-bench logs will be saved to: {:?}", log_file_path);

        // Build the evm-bench command
        let mut cmd = Command::new("evm-bench");
        cmd.args([
            "new-payload-fcu",
            "--rpc-url",
        ...
        debug!("Executing evm-bench command: {:?}", cmd);

        // Execute the benchmark
        let mut child = cmd.spawn().wrap_err("Failed to start evm-bench process")?;
        ...
        if let Some(stdout) = child.stdout.take() {
            ...
            tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    debug!("[EVM-BENCH] {}", line);
                    if let Ok(mut captured) = stdout_lines_clone.lock() {
                        captured.push(line.clone());
                    }
                    // Write to log file (evm-bench output already has timestamps if needed)
                    let log_line = format!("{}\n", line);
        ...
        let status = child.wait().await.wrap_err("Failed to wait for evm-bench")?;

        if !status.success() {
            // Print all captured output when command fails
            error!("evm-bench failed with exit code: {:?}", status.code());

            if let Ok(stdout) = stdout_lines.lock() &&
                !stdout.is_empty()
            {
                error!("evm-bench stdout:");
            ...
            if let Ok(stderr) = stderr_lines.lock() &&
                !stderr.is_empty()
            {
                error!("evm-bench stderr:");
                for line in stderr.iter() {
                    error!("  {}", line);
                }
            }

            return Err(eyre!("evm-bench failed with exit code: {:?}", status.code()));
        }
