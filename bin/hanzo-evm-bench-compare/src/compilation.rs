//! Compilation operations for evm and evm-bench.
...
/// Manages compilation operations for evm components
#[derive(Debug)]
...
impl CompilationManager {
    ...
    pub(crate) fn get_cached_binary_path_for_commit(&self, commit: &str) -> PathBuf {
        ...
        self.output_dir.join("bin").join(format!("evm_{identifier}"))
    }

    /// Compile evm using cargo build and cache the binary
    ...
    /// Check if evm-bench is available in PATH
    pub(crate) fn is_hanzo_evm_bench_available(&self) -> bool {
        match Command::new("which").arg("evm-bench").output() {
            Ok(output) => {
                if output.status.success() {
                    let path = String::from_utf8_lossy(&output.stdout);
                    info!("Found evm-bench: {}", path.trim());
                    true
                } else {
                    false
                }
    ...
    /// Ensure evm-bench is available, compiling if necessary
    pub(crate) fn ensure_hanzo_evm_bench_available(&self) -> Result<()> {
        if self.is_hanzo_evm_bench_available() {
            Ok(())
        } else {
            warn!("evm-bench not found in PATH, compiling and installing...");
            self.compile_hanzo_evm_bench()
        }
    }

    /// Compile and install evm-bench using `make install-evm-bench`
    pub(crate) fn compile_hanzo_evm_bench(&self) -> Result<()> {
        info!("Compiling and installing evm-bench...");

        let mut cmd = Command::new("make");
        cmd.arg("install-evm-bench").current_dir(&self.repo_root);

        info!("Compiling evm-bench with {cmd:?}");

        let output = cmd.output().wrap_err("Failed to execute make install-evm-bench command")?;
        ...
        if !output.status.success() {
            // Print all output when compilation fails
            error!("Make install-evm-bench failed with exit code: {:?}", output.status.code());
            ...
            return Err(eyre!(
                "evm-bench compilation failed with exit code: {:?}",
                output.status.code()
            ));
        }
