...
impl ComparisonGenerator {
    ...
    /// Set the evm command for a reference
    ...
    pub(crate) async fn generate_comparison_report(&self) -> Result<()> {
        ...
        let report = ComparisonReport {
            ...
            baseline: RefInfo {
                ...
                evm_command: self.baseline_command.clone(),
            },
            feature: RefInfo {
                ...
                evm_command: self.feature_command.clone(),
            },
            comparison_summary,
            per_block_comparisons,
        };
    ...
    fn print_comparison_summary(&self, report: &ComparisonReport) {
        ...
        if let Some(ref cmd) = report.baseline.evm_command {
            println!("  Command: {}", cmd);
        }
        println!();
        ...
        if let Some(ref cmd) = report.feature.evm_command {
            println!("  Command: {}", cmd);
        }
        println!();
    }
