//! Node management for starting, stopping, and controlling evm instances.
...
/// Default websocket RPC port used by evm
const DEFAULT_WS_RPC_PORT: u16 = 8546;

/// Manages evm node lifecycle and operations
pub(crate) struct NodeManager {
    ...
    output_dir: PathBuf,
    additional_evm_args: Vec<String>,
...
impl NodeManager {
    ...
    pub(crate) fn new(args: &Args) -> Self {
        Self {
            ...
            // Filter out empty strings to prevent invalid arguments being passed to evm node
            additional_evm_args: args
                .evm_args
                .iter()
                .filter(|s| !s.is_empty())
                .cloned()
    ...
    fn get_log_file_path(&self, ref_type: &str) -> Result<PathBuf> {
        ...
        let log_file = log_dir.join("hanzo_evm_node.log");
        Ok(log_file)
    }
    ...
    /// Build evm arguments as a vector of strings
    fn build_evm_args(
        &self,
        binary_path_str: &str,
        ...
        if let Some(ref endpoint) = self.tracing_endpoint {
            ...
            // Endpoint requires equals per clap settings in evm
            evm_args.push(format!("--tracing-otlp={}", endpoint));
        }

        // Add any additional arguments passed via command line (common to both baseline and
        // feature)
        evm_args.extend_from_slice(&self.additional_evm_args);

        // Add reference-specific additional arguments
        evm_args.extend_from_slice(additional_args);
    ...
    async fn create_profiling_command(
        ...
        info!("Starting evm node with samply profiling...");
        ...
        // Add separator and complete evm command
        cmd.arg("--");
        cmd.args(evm_args);

        // Enable tracing-samply
        if supports_samply_flags(&evm_args[0]) {
            cmd.arg("--log.samply");
        }
    ...
    /// Create a command for direct evm execution
    fn create_direct_command(&self, evm_args: &[String]) -> Command {
        let binary_path = &evm_args[0];

        let mut cmd = if self.use_sudo {
            info!("Starting evm node with sudo...");
            let mut sudo_cmd = Command::new("sudo");
            sudo_cmd.args(evm_args);
            sudo_cmd
        } else {
            info!("Starting evm node...");
            let mut evm_cmd = Command::new(binary_path);
            evm_cmd.args(&evm_args[1..]); // Skip the binary path since it's the command
            evm_cmd
        };
    ...
    /// Start a evm node using the specified binary path and return the process handle
    /// along with the formatted evm command string for reporting.
    pub(crate) async fn start_node(
        ...
        let (evm_args, _) = self.build_evm_args(&binary_path_str, additional_args, ref_type);

        // Format the evm command string for reporting
        let evm_command = shlex::try_join(evm_args.iter().map(|s| s.as_str()))
            .wrap_err("Failed to format evm command string")?;

        // Log additional arguments if any
        if !self.additional_evm_args.is_empty() {
            info!("Using common additional evm arguments: {:?}", self.additional_evm_args);
        }
        if !additional_args.is_empty() {
            info!("Using reference-specific additional evm arguments: {:?}", additional_args);
        }

        let mut cmd = if self.enable_profiling {
            self.create_profiling_command(ref_type, &evm_args).await?
        } else {
            self.create_direct_command(&evm_args)
        };
        ...
        debug!("Executing evm command: {cmd:?}");
        ...
        let mut child = cmd
            ...
            .spawn()
            .wrap_err("Failed to start evm node")?;

        info!(
            "Hanzo EVM node started with PID: {:?} (binary: {})",
            child.id().ok_or_eyre("Hanzo EVM node is not running")?,
            binary_path_str
        );

        // Prepare log file path
        let log_file_path = self.get_log_file_path(ref_type)?;
        info!("Hanzo EVM node logs will be saved to: {:?}", log_file_path);
    ...
    pub(crate) async fn stop_node(&self, child: &mut tokio::process::Child) -> Result<()> {
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                info!("Hanzo EVM node (PID: {}) has already exited with status: {:?}", pid, status);
                return Ok(());
            }
            Ok(None) => {
        ...
        match child.wait().await {
            Ok(status) => {
                info!("Hanzo EVM node (PID: {}) exited with status: {:?}", pid, status);
            }
            Err(e) => {
    ...
    pub(crate) async fn unwind_to_block(&self, block_number: u64) -> Result<()> {
        ...
        debug!("Executing evm unwind command: {:?}", cmd);
