# Benchmarking reth live sync with `evm-bench`

The binary contained in this directory, `evm-bench`, is a tool that can be used to benchmark the performance of the reth live sync. `evm-bench` is a general tool, and can be used for benchmarking node performance, as long as the node supports the engine API.
...
### The `evm-bench` tool
The `evm-bench` tool is designed to benchmark performance of reth live sync.
...
`evm-bench` contains different commands to benchmark different patterns of engine API calls.
The `evm-bench new-payload-fcu` command is the most representative of ethereum mainnet live sync, alternating between sending `engine_newPayload` calls and `engine_forkchoiceUpdated` calls.
...
- `--wait-for-persistence`: Waits for blocks to be persisted using the `evm_subscribePersistedBlock` subscription
...
Any consensus layer client configured to connect to `reth` should be shut down, as all the engine API interactions during benchmarking will be driven by `evm-bench`.
...
The following `evm-bench` command would then start the benchmark at block 21,000,000:
```bash
evm-bench new-payload-fcu --rpc-url <rpc-url> --from 21000000 --to <end_block> --jwt-secret <jwt_file_path>
```
...
```bash
evm-bench new-payload-fcu --rpc-url <rpc-url> --from <start_block> --to <end_block> --jwt-secret <jwt_file_path>
```
...
Note that this assumes that the benchmark node's engine API is running on `http://127.0.0.1:8551`, which is set as a default value in `evm-bench`. To configure this value, use the `--engine-rpc-url` flag.
...
evm-bench new-payload-fcu --advance 10 --jwt-secret <jwt_file_path> --rpc-url <rpc-url>

# Benchmark the next 50 blocks with a different subcommand
evm-bench new-payload-only --advance 50 --jwt-secret <jwt_file_path> --rpc-url <rpc-url>
```

### Observe Outputs

After running the command, `evm-bench` will output benchmark results, showing processing speeds and gas usage, which are useful metrics for analyzing the node's performance.
...
- **Benchmark Data**: `evm-bench` additionally contains a `--output` flag, which will output gas used benchmarks across the benchmark range in CSV format. This may be useful for further data analysis.
