//! Runs the `evm bench` command, sending only newPayload, without a forkchoiceUpdated call.
...
/// `evm benchmark new-payload-only` command
#[derive(Debug, Parser)]
pub struct Command {
...
impl Command {
    ...
    pub async fn execute(self, _ctx: CliContext) -> eyre::Result<()> {
        ...
        while let Some(block) = {
            ...
            debug!(target: "evm-bench", number=?block.header.number, "Sending payload to engine");
            ...
            info!(target: "evm-bench", %new_payload_result);
        ...
        if let Some(path) = self.benchmark.output {
            ...
            info!(target: "evm-bench", "Writing newPayload call latency output to file: {:?}", output_path);
            ...
            info!(target: "evm-bench", "Writing total gas output to file: {:?}", output_path);
            let mut writer = Writer::from_path(output_path)?;
            ...
            info!(target: "evm-bench", "Finished writing benchmark output files to {:?}.", path);
        }
