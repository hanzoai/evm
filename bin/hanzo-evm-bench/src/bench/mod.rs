//! `evm benchmark` command. Collection of various benchmarking routines.

use clap::{Parser, Subcommand};
use hanzo_evm_cli_runner::CliContext;
use hanzo_evm_node_core::args::LogArgs;
...
pub enum Subcommands {
    ...
    ///
    /// `cast block latest --full --json | evm-bench send-payload --rpc-url localhost:5000
    ...
    /// Example:
    ///
    /// `evm-bench generate-big-block --rpc-url http://localhost:8545 --engine-rpc-url
    ...
    /// Example:
    ///
    /// `evm-bench replay-payloads --payload-dir ./payloads --engine-rpc-url
    ...
    /// Example:
    ///
    /// `cast block latest --full --json | evm-bench send-invalid-payload --rpc-url localhost:5000
...
impl BenchmarkCommand {
    ...
    ///
    /// Always enables log target display (`RUST_LOG_TARGET=1`) so that the `evm-bench` target
    /// is visible in output, making it easy to distinguish evm-bench logs from evm logs when
    /// both are streamed to the same console or file.
    pub fn init_tracing(&self) -> eyre::Result<Option<FileWorkerGuard>> {
        // Always show the log target so "evm-bench" is visible in the output.
