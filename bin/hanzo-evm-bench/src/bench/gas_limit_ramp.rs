...
use hanzo_evm_primitives_traits::constants::{GAS_LIMIT_BOUND_DIVISOR, MAXIMUM_GAS_LIMIT_BLOCK};
use std::{path::PathBuf, time::Instant};
use tracing::info;

/// `evm benchmark gas-limit-ramp` command.
#[derive(Debug, Parser)]
pub struct Command {
...
impl Command {
    ...
    pub async fn execute(self, _ctx: CliContext) -> eyre::Result<()> {
        ...
        if !self.output.exists() {
            ...
            info!(target: "evm-bench", "Created output directory: {:?}", self.output);
        }
        ...
        while !should_stop(mode, blocks_processed, parent_header.gas_limit) {
            ...
            info!(target: "evm-bench", block_number = block.header.number, path = %payload_path.display(), "Saved payload");
