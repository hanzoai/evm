...
impl BenchContext {
    ...
    pub(crate) async fn new(bench_args: &BenchmarkArgs, rpc_url: String) -> eyre::Result<Self> {
        info!(target: "evm-bench", "Running benchmark using data from RPC URL: {}", rpc_url);
        ...
        if let Some(output) = &bench_args.output {
            ...
            if !output.exists() {
                ...
                info!(target: "evm-bench", "Created output directory: {:?}", output);
            }
        }
        ...
        info!(target: "evm-bench", "Connecting to Engine RPC at {} for replay", auth_url);
