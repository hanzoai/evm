...
//!   `evm_subscribePersistedBlock` subscription
...
/// Default `WebSocket` RPC port for evm.
...
///
/// The persistence subscription endpoint (`evm_subscribePersistedBlock`) is exposed on
...
pub(crate) fn derive_ws_rpc_url(
    ...
    if let Some(ws_url) = ws_rpc_url {
        ...
        info!(target: "evm-bench", ws_url = %parsed, "Using provided WebSocket RPC URL");
        Ok(parsed)
    } else {
...
/// - Port is always set to `8546`, evm's default RPC websocket port.
///
...
/// Establishes a websocket connection and subscribes to `evm_subscribePersistedBlock`.
pub(crate) async fn setup_persistence_subscription(
    ws_url: Url,
) -> eyre::Result<PersistenceSubscription> {
    info!(target: "evm-bench", "Connecting to WebSocket at {} for persistence subscription", ws_url);
    ...
    let subscription = provider
        .subscribe_to::<BlockNumHash>("evm_subscribePersistedBlock")
        .await
        .wrap_err("Failed to subscribe to persistence notifications")?;

    info!(target: "evm-bench", "Subscribed to persistence notifications");
