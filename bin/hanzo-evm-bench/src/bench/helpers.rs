//! Common helpers for evm-bench commands.
...
pub(crate) fn read_input(path: Option<&str>) -> Result<String> {
    Ok(match path {
        Some(path) => hanzo_evm_fs_util::read_to_string(path)?,
...
pub(crate) async fn get_payload_with_sidecar(
    ...
    debug!(target: "evm-bench", get_payload_version = ?version, ?payload_id, "Sending getPayload");
