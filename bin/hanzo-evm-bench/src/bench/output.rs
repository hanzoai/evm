...
use csv::Writer;
use eyre::OptionExt;
use hanzo_evm_primitives_traits::constants::GIGAGAS;
...
pub(crate) fn write_benchmark_results(
    ...
    info!(target: "evm-bench", "Writing engine api call latency output to file: {:?}", output_path);
    ...
    info!(target: "evm-bench", "Finished writing benchmark output files to {:?}.", output_dir);
    Ok(())
}
