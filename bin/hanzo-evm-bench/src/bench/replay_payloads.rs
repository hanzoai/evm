...
//!   `evm_subscribePersistedBlock` subscription, where N matches the engine's persistence
...
use hanzo_evm_engine_primitives::config::DEFAULT_PERSISTENCE_THRESHOLD;
use hanzo_evm_node_api::EngineApiMessageVersion;
use std::{
    path::PathBuf,
...
/// `evm bench replay-payloads` command
///
...
impl Command {
    ...
    pub async fn execute(self, _ctx: CliContext) -> eyre::Result<()> {
        info!(target: "evm-bench", payload_dir = %self.payload_dir.display(), "Replaying payloads");

        // Log mode configuration
        if let Some(duration) = self.wait_time {
            info!(target: "evm-bench", "Using wait-time mode with {}ms delay between blocks", duration.as_millis());
        }
        ...
        let gas_ramp_payloads = if let Some(ref gas_ramp_dir) = self.gas_ramp_dir {
            ...
            info!(target: "evm-bench", count = payloads.len(), "Loaded gas ramp payloads from disk");
            payloads
        } else {
            Vec::new()
        };
        ...
        info!(target: "evm-bench", count = payloads.len(), "Loaded main payloads from disk");
        ...
        for (i, payload) in gas_ramp_payloads.iter().enumerate() {
            ...
            info!(target: "evm-bench", gas_ramp_payload = i + 1, "Gas ramp payload executed successfully");
        ...
        if !gas_ramp_payloads.is_empty() {
            info!(target: "evm-bench", count = gas_ramp_payloads.len(), "All gas ramp payloads replayed");
        }
        ...
        for (i, payload) in payloads.iter().enumerate() {
            ...
            debug!(target: "evm-bench", method = "engine_forkchoiceUpdatedV3", ?fcu_state, "Sending forkchoiceUpdated");
            ...
            info!(target: "evm-bench", %combined_result);
            ...
            debug!(target: "evm-bench", ?status, ?fcu_result, "Payload executed successfully");
