    ...
    pub async fn collect_gas(
        ...
        while total_gas < gas_target {
            let Some((block_txs, _)) = self.source.fetch_block_transactions(current_block).await?
                ...
                warn!(target: "evm-bench", block = current_block, "Block not found, stopping");
                break;
            };
...
/// `evm bench generate-big-block` command
///
        ...
        match collector.collect(block).await {
            ...
            Err(e) => {
                if attempt == MAX_FETCH_RETRIES {
                    warn!(target: "evm-bench", attempt, error = %e, "Failed to fetch transactions after max retries");
                    return None;
                }
                warn!(target: "evm-bench", attempt, error = %e, "Failed to fetch transactions, retrying...");
...
impl Command {
    ...
    pub async fn execute(self, _ctx: CliContext) -> eyre::Result<()> {
        info!(target: "evm-bench", target_gas = self.target_gas, count = self.count, "Generating big block(s)");
        ...
        info!(target: "evm-bench", "Connecting to Engine RPC at {}", auth_url);
        ...
        info!(target: "evm-bench", "Connecting to Testing RPC at {}", self.testing_rpc_url);
        ...
        info!(target: "evm-bench", endpoint = "engine", method = "eth_getBlockByNumber", block = "latest", "RPC call");
        ...
        info!(target: "evm-bench", count = self.count, output_dir = %self.output_dir.display(), "All payloads generated");
        Ok(())
    }
    ...
    async fn execute_sequential_with_retry<S: TransactionSource>(
        ...
        for i in 0..self.count {
            ...
            if self.execute || self.count > 1 {
                info!(target: "evm-bench", payload = i + 1, block_hash = %built.block_hash, gas_used = built.gas_used, "Executing payload (newPayload + FCU)");
                self.execute_payload_v4(auth_provider, built.envelope, parent_hash).await?;
                info!(target: "evm-bench", payload = i + 1, "Payload executed successfully");
            }
    ...
    async fn build_with_retry<S: TransactionSource>(
        ...
        warn!(target: "evm-bench", payload = index + 1, "Retry loop exited without returning a payload");
    ...
    async fn execute_pipelined(
        ...
        let fetcher_handle = tokio::spawn(async move {
            let tx_source = match RpcTransactionSource::from_url(&rpc_url) {
                Ok(source) => source,
                Err(e) => {
                    warn!(target: "evm-bench", error = %e, "Failed to create transaction source");
                    return None;
                }
            };
            ...
            while let Some(batch) = fetch_batch_with_retry(&collector, current_block).await {
                if batch.transactions.is_empty() {
                    info!(target: "evm-bench", block = current_block, "Reached chain tip, stopping fetcher");
                    break;
                }
        ...
        for i in 0..self.count {
            ...
            // Execute payload
            info!(target: "evm-bench", payload = i + 1, block_hash = %current_block_hash, gas_used = built.gas_used, "Executing payload (newPayload + FCU)");
            self.execute_payload_v4(auth_provider, built.envelope, parent_hash).await?;
            info!(target: "evm-bench", payload = i + 1, "Payload executed successfully");
                ...
                RetryOutcome::NeedMore(additional_gas) => {
                    ...
                    while collected_gas < additional_gas {
                        if let Some(batch) = tx_buffer.take_batch().await {
                            ...
                            warn!(target: "evm-bench", "Transaction fetcher exhausted, proceeding with available transactions");
                            break;
                        }
                    }
                }
    ...
    fn save_payload(&self, payload: &BuiltPayload) -> eyre::Result<()> {
        ...
        info!(target: "evm-bench", block_number = payload.block_number, block_hash = %payload.block_hash, path = %filepath.display(), "Payload saved");
        Ok(())
    }
