//! Runs the `evm bench` command, calling first newPayload for each block, then calling
//! forkchoiceUpdated.
//!
...
use hanzo_evm_node_core::args::BenchmarkArgs;
use std::time::{Duration, Instant};
use tracing::{debug, info};

/// `evm benchmark new-payload-fcu` command
#[derive(Debug, Parser)]
pub struct Command {
...
impl Command {
    ...
    pub async fn execute(self, _ctx: CliContext) -> eyre::Result<()> {
        ...
        tokio::task::spawn(async move {
            while benchmark_mode.contains(next_block) {
                ...
                let block = match block_res.and_then(|opt| opt.ok_or_eyre("Block not found")) {
                    Ok(block) => block,
                    Err(e) => {
                        tracing::error!(target: "evm-bench", "Failed to fetch block {next_block}: {e}");
                ...
                if let Err(e) = sender
                    ...
                    .await
                {
                    tracing::error!(target: "evm-bench", "Failed to send block data: {e}");
                    break;
                }
            }
        });
        ...
        while let Some((block, head, safe, finalized)) = {
            ...
            debug!(target: "evm-bench", ?block_number, "Sending payload");
