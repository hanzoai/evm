use hanzo_evm_ethereum_primitives::EthPrimitives;
use hanzo_evm_execution::ConfigureEvm;
use hanzo_evm_network::{protocol::IntoRlpxSubProtocol, NetworkProtocols};
use hanzo_evm_network_api::FullNetwork;
use hanzo_evm_node_api::ConsensusEngineEvent;
use hanzo_evm_node_core::args::RessArgs;
use hanzo_evm_provider::providers::{BlockchainProvider, ProviderNodeTypes};
use hanzo_evm_ress_protocol::{NodeType, ProtocolState, RessProtocolHandler};
use hanzo_evm_ress_provider::{maintain_pending_state, PendingState, EvmRessProtocolProvider};
use hanzo_evm_tasks::TaskExecutor;
use hanzo_evm_tokio_util::EventStream;
use tokio::sync::mpsc;
use tracing::*;

/// Install `ress` subprotocol if it's enabled.
pub fn install_ress_subprotocol<P, E, N>(
    args: RessArgs,
    provider: BlockchainProvider<P>,
    hanzo_evm_config: E,
    network: N,
    task_executor: TaskExecutor,
    engine_events: EventStream<ConsensusEngineEvent<EthPrimitives>>,
) -> eyre::Result<()>
where
    P: ProviderNodeTypes<Primitives = EthPrimitives>,
    E: ConfigureEvm<Primitives = EthPrimitives> + Clone + 'static,
    N: FullNetwork + NetworkProtocols,
{
    info!(target: "evm::cli", "Installing ress subprotocol");
    let pending_state = PendingState::default();

    // Spawn maintenance task for pending state.
    task_executor.spawn(maintain_pending_state(
        engine_events,
        provider.clone(),
        pending_state.clone(),
    ));

    let (tx, mut rx) = mpsc::unbounded_channel();
    let provider = EvmRessProtocolProvider::new(
        provider,
        hanzo_evm_config,
        Box::new(task_executor.clone()),
        args.max_witness_window,
        args.witness_max_parallel,
        args.witness_cache_size,
        pending_state,
    )?;
    network.add_rlpx_sub_protocol(
        RessProtocolHandler {
            provider,
            node_type: NodeType::Stateful,
            peers_handle: network.peers_handle().clone(),
            max_active_connections: args.max_active_connections,
            state: ProtocolState::new(tx),
        }
        .into_rlpx_sub_protocol(),
    );
    info!(target: "evm::cli", "Ress subprotocol support enabled");

    task_executor.spawn(async move {
        while let Some(event) = rx.recv().await {
            trace!(target: "evm::ress", ?event, "Received ress event");
        }
    });
    Ok(())
}
