...
mod tests {
    ...
    struct EvmAccessList(Vec<EvmAccessListItem>);

    impl PartialEq<AccessList> for EvmAccessList {
    ...
    impl PartialEq<AccessListItem> for EvmAccessListItem {
    ...
    proptest!(
        #[test]
        fn test_roundtrip_accesslist_compat(access_list in arb::<EvmAccessList>()) {
            // Convert access_list to buffer and then create alloy_access_list from buffer and
            // compare
            let mut compacted_evm_access_list = Vec::<u8>::new();
            let len = access_list.to_compact(&mut compacted_evm_access_list);

            // decode the compacted buffer to AccessList
            let alloy_access_list = AccessList::from_compact(&compacted_evm_access_list, len).0;
            ...
            assert_eq!(compacted_evm_access_list, compacted_alloy_access_list);
        }
    );
}
