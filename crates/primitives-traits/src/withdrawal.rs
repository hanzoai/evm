...
mod tests {
    ...
    impl PartialEq<Withdrawal> for EvmWithdrawal {
    ...
    // <https://github.com/hanzoai/evm/issues/1614>
    #[test]
    ...
    proptest!(
        #[test]
        fn test_roundtrip_withdrawal_compat(withdrawal in arb::<EvmWithdrawal>()) {
            // Convert to buffer and then create alloy_access_list from buffer and
            // compare
            let mut compacted_evm_withdrawal = Vec::<u8>::new();
            let len = withdrawal.to_compact(&mut compacted_evm_withdrawal);

            // decode the compacted buffer to AccessList
            let alloy_withdrawal = Withdrawal::from_compact(&compacted_evm_withdrawal, len).0;
            ...
            assert_eq!(compacted_evm_withdrawal, compacted_alloy_withdrawal);
        }
    );
}
