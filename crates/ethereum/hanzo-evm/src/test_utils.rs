...
use hanzo_evm_execution_types::{BlockExecutionResult, ExecutionOutcome};
use hanzo_evm_primitives_traits::{BlockTy, SealedBlock, SealedHeader};
use revm::{
...
pub struct MockExecutor<'a, DB: Database, I> {
    ...
    evm: EthEvm<&'a mut State<DB>, I, PrecompilesMap>,
    #[debug(skip)]
    hook: Option<Box<dyn hanzo_evm_execution::OnStateHook>>,
    ...
    for MockExecutor<'a, DB, I>
{
    ...
    fn set_state_hook(&mut self, hook: Option<Box<dyn hanzo_evm_execution::OnStateHook>>) {
        self.hook = hook;
    }
...
impl ConfigureEvm for MockEvmConfig {
    ...
    ) -> Result<hanzo_evm_execution::ExecutionCtxFor<'a, Self>, Self::Error> {
    ...
    fn context_for_next_block(
        ...
        parent: &SealedHeader,
        attributes: Self::NextBlockEnvCtx,
    ) -> Result<hanzo_evm_execution::ExecutionCtxFor<'_, Self>, Self::Error> {
