...
Currently `Compact` is used when adding the derive macro `evm_codec`.

This crate implements the main codec (`Compact`) for:
* [primitive types](src/lib.rs)
* [alloy types](src/alloy/mod.rs): uses bridge types alongside `evm_codec` from [derive](derive/src/lib.rs)

### evm-codecs-derive

Provides derive macros that can be added and configured to stored data structs/enums
* `#[evm_codec]`: Implements `Compact` as well as `#[derive_arbitrary(compact)]`
* `#[evm_codec(rlp)]`: Implements `Compact` as well as `#[derive_arbitrary(compact, rlp)]`
* `#[evm_codec(no_arbitrary)]`: Implements `Compact` without `derive_arbitrary`.
