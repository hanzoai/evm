...
//! `hanzo_evm_rpc_eth_api::helpers::Call`.

use alloy_primitives::{Address, B256, U256};
use hanzo_evm_errors::ProviderResult;
use hanzo_evm_revm::database::StateProviderDatabase;
use hanzo_evm_storage_api::{BytecodeReader, HashedPostStateProvider, StateProvider, StateProviderBox};
use hanzo_evm_trie::{HashedStorage, MultiProofTargets};
...
impl hanzo_evm_storage_api::StateRootProvider for StateProviderTraitObjWrapper {
    fn state_root(
        &self,
        hashed_state: hanzo_evm_trie::HashedPostState,
    ) -> hanzo_evm_errors::ProviderResult<B256> {
        self.0.state_root(hashed_state)
    }

    fn state_root_from_nodes(
        &self,
        input: hanzo_evm_trie::TrieInput,
    ) -> hanzo_evm_errors::ProviderResult<B256> {
    ...
    fn state_root_with_updates(
        &self,
        hashed_state: hanzo_evm_trie::HashedPostState,
    ) -> hanzo_evm_errors::ProviderResult<(B256, hanzo_evm_trie::updates::TrieUpdates)> {
...
impl hanzo_evm_storage_api::StorageRootProvider for StateProviderTraitObjWrapper {
    fn storage_root(
        &self,
        address: Address,
    ...
    fn storage_multiproof(
        ...
        slots: &[B256],
        hashed_storage: HashedStorage,
    ) -> ProviderResult<hanzo_evm_trie::StorageMultiProof> {
        self.0.storage_multiproof(address, slots, hashed_storage)
    }
}

impl hanzo_evm_storage_api::StateProofProvider for StateProviderTraitObjWrapper {
    fn proof(
        &self,
        input: hanzo_evm_trie::TrieInput,
        address: Address,
        slots: &[B256],
    ) -> hanzo_evm_errors::ProviderResult<hanzo_evm_trie::AccountProof> {
    ...
    ) -> ProviderResult<hanzo_evm_trie::MultiProof> {
    ...
    fn witness(
        &self,
        input: hanzo_evm_trie::TrieInput,
        target: hanzo_evm_trie::HashedPostState,
    ) -> hanzo_evm_errors::ProviderResult<Vec<alloy_primitives::Bytes>> {
        self.0.witness(input, target)
    }
}

impl hanzo_evm_storage_api::AccountReader for StateProviderTraitObjWrapper {
    fn basic_account(
        &self,
        address: &Address,
    ) -> hanzo_evm_errors::ProviderResult<Option<hanzo_evm_primitives_traits::Account>> {
        self.0.basic_account(address)
    }
}

impl hanzo_evm_storage_api::BlockHashReader for StateProviderTraitObjWrapper {
    fn block_hash(
        &self,
        block_number: alloy_primitives::BlockNumber,
    ) -> hanzo_evm_errors::ProviderResult<Option<B256>> {
    ...
    ) -> hanzo_evm_errors::ProviderResult<Vec<B256>> {
        self.0.canonical_hashes_range(start, end)
    }
}

impl HashedPostStateProvider for StateProviderTraitObjWrapper {
    fn hashed_post_state(&self, bundle_state: &BundleState) -> hanzo_evm_trie::HashedPostState {
...
impl StateProvider for StateProviderTraitObjWrapper {
    ...
    ) -> hanzo_evm_errors::ProviderResult<Option<alloy_primitives::StorageValue>> {
    ...
    fn account_code(
        &self,
        addr: &Address,
    ) -> hanzo_evm_errors::ProviderResult<Option<hanzo_evm_primitives_traits::Bytecode>> {
        self.0.account_code(addr)
    }

    fn account_balance(&self, addr: &Address) -> hanzo_evm_errors::ProviderResult<Option<U256>> {
        self.0.account_balance(addr)
    }

    fn account_nonce(&self, addr: &Address) -> hanzo_evm_errors::ProviderResult<Option<u64>> {
