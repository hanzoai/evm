...
extern "C" fn print_stack_trace(_: libc::c_int) {
    ...
    raw_errln!("error: evm interrupted by SIGSEGV, printing backtrace\n");
    let mut written = 1;
    let mut consumed = 0;
    ...
    if cyclic || stack.len() > random_depth() {
        ...
        raw_errln!("note: evm unexpectedly overflowed its stack! this is a bug");
        written += 1;
    }
    ...
    raw_errln!("note: we would appreciate a report at https://github.com/hanzoai/evm");
    written += 1;
    if written > 24 {
